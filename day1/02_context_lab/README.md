...
### Step 3. knowledge.md 직접 작성 (Copilot Ask 모드 활용)
1. VS Code에서 Copilot Chat을 **Ask 모드**로 엽니다.
2. `knowledge.md` 파일을 열고, 안내된 [작성 방법]에 따라 Ask 모드와 대화하며 항목을 채워봅니다.
   막막하면 이렇게 물어보세요.
   ```
   ...
   같이 채워줘.
   ```
3. Ask 모드가 제안한 초안을 그대로 쓰지 말고, 실제 가르치는(또는 가르친다고 가정한) 상황에 맞게 직접 다듬어서 `knowledge.md`에 채워 넣습니다.
...
그리고 이 knowledge.md 를 채우는 방식(Ask 모드로 상황을 설명하고 함께 정리하기)은
Day2 Custom Instructions 실습에서도 그대로 이어집니다.
