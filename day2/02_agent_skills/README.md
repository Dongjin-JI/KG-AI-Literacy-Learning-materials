...
```
할 일 개수를 반환하는 통계 API를 추가해줘
```
→ 결과를 확인하세요. 기존 파일 역할 분리(main.py/database.py), 응답 형식이 일관되게 지켜졌나요?
...
3. 본문의 빈칸(파일 역할 분리, 응답 형식, 반드시 지킬 것)을 채웁니다.
...
4. 터미널에서 직접 실행해서 결과를 비교해볼 수도 있습니다.
   ```bash
   python .github/skills/code-style-check/scripts/check_style.py database.py
   ```
